...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    {
      for {
        (char, letters) <- w.toLowerCase.groupBy(char => char)

      } yield (char, letters.length)
    }.toList.sortWith((value1, value2) => value1._1 < value2._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    def add(acc: Map[Char, Int], value: (Char, Int)): Map[Char, Int] = {
      val (char, quantity) = value

      acc + (char -> (xMap(char) - quantity))

    }

    y.toMap.foldLeft(xMap)(add).filterNot(_._2 == 0).toList.sortWith((x, y) => x._1 < y._1)
  }

  def subtractOther(y: Occurrences, x: Occurrences): Occurrences =
    ...
    def occurrencesAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          rest <- occurrencesAnagrams(subtract(occurrences, occ))
        } yield w :: rest
      }.toList
    }
