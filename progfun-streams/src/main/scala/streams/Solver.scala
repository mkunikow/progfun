...
trait Solver extends GameDef {
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(List((startBlock,List())).toStream,Set())
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.take(1) match {
